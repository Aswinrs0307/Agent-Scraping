# filename: save_filtered_results.py
...
# Prepare to save the filtered results to a text file
with open("filtered_search_results.txt", "w", encoding="utf-8") as output_file:
    for result in results:
        columns = result.find_all("td")  # Assuming data is in table cells
        if columns:
            data = [col.get_text(strip=True) for col in columns]
            # Check if the name "BLACK JACK SPORTS BETTING" is in the data
            if "BLACK JACK SPORTS BETTING" in data:
                # Write the filtered data to the text file
                output_file.write(" | ".join(data) + "\n")

print("Filtered search results saved to filtered_search_results.txt.")